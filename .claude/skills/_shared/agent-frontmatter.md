...
Referencia para los 17 native sub-agents de `.claude/agents/sdd-*.md` y los 9 skill routers de `.claude/skills/<phase>/SKILL.md`. Patrón validado en T3 pilot (2026-04-23) y alineado con gentle-ai v1.23.

## Por qué dos archivos por fase

Los slash commands (`/plan-feature`, `/archive-feature`, etc.) siempre rutean al skill, nunca al agent nativo directamente. Para que el user obtenga aislamiento de contexto + model-per-phase sin cambiar de hábito:

- **Skill** = router de ~10 líneas que delega al agent nativo (cuando está disponible)
- **Agent** = executor con la lógica completa de la fase en su body + frontmatter declarando model/tools

Así: `/plan-feature 008` → skill router → main Claude invoca `Agent(subagent_type="sdd-plan-feature", prompt="feature-id: 008")` → agent nativo corre en su modelo, aislado.
...
- **`disallowedTools: [Agent]`** en los 15 executors para impedir recursión. Omitir en `sdd-plan-feature` y `sdd-review-feature` (conservan Agent tool para delegar a sub-agents internos).
...
| `disallowedTools` | ⭕ | ej. `[Agent]` | Omitir para `sdd-plan-feature` y `sdd-review-feature` |
...
**9 public agents** (con skill router): `sdd-<phase>` donde phase = research-spike, new-feature, new-quick-feature, new-fix, plan-feature, implement-task, simplify-code, review-feature, archive-feature.

**8 internal agents** (no skill router, spawn desde plan-feature/review-feature): `sdd-explore-agent`, `sdd-discovery-evaluator`, `sdd-designer`, `sdd-task-planner`, `sdd-reviewer-voter` (3x), `sdd-adversarial-reviewer`.
