...
The agent runs in sonnet (executor — `disallowedTools: [Agent]`) and executes one unlocked `[AFK]` vertical slice per invocation (TDD detection, dependency checks, task writeback, validation, delta capture, Engram save). See `.claude/agents/sdd-implement-task.md` for the full body.
