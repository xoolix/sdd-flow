...
Parse `$ARGUMENTS` for flags and determine the review tier. This step runs after state-read (Step 1.5) but before launching voters (Step 3).
...
### 4. Collect verdicts and apply voting logic

**If `voter_count == 1` (tier = `minimal` or `fast-lane`)**:

The single voter's verdict IS the review verdict — no aggregation. Skip the voting matrix below and proceed directly to Step 5 using Agent-A's output as the sole source of truth.
...
### 5. Build consolidated review report
...
### 5.5. Invalidate simplify sentinel on conformance FAIL
...
**Scope**: this deletion fires **only on conformance FAIL**. PASS and PASS WITH WARNINGS leave the sentinel intact. SPEC-GAP-HIGH (Step 6.5) also leaves the sentinel intact — that path pauses for a human spec edit, not a code fix.

### 6. Build Review-Feedback (when verdict is FAIL or PASS WITH WARNINGS)
...
### 6.5. Adversarial spec review (runs only when conformance verdict is PASS or PASS WITH WARNINGS AND adversarial_enabled is true)

**Tier gate**: Skip this step entirely if `adversarial_enabled == false` (tier = `minimal`). When skipped, proceed directly to Step 7.
...
- The consolidated conformance report from Step 5
...
|-------------------|---------------------|
| No gaps (output says "None") | Continue — proceed to Step 7 (Engram save) |
| Only medium/low severity gaps | Write gaps to `decisions.md` with `SPEC-GAP` tag (format below); continue to Step 7 |
| Any high-severity gap present | Write gaps to `decisions.md` with `SPEC-GAP-HIGH` tag (format below); stop pipeline — run Step 7 (Engram save) BEFORE returning `Status: blocked` with `Spec-Gaps` field in result envelope |
...
**Blocked path — Engram save**: Before returning a `Status: blocked` envelope (high-severity gap found), the orchestrator MUST still run Step 7 (Engram save). This ensures conformance review findings are persisted even when the adversarial agent stops the pipeline.
...
- **On start** (Step 1): `mem_search` query `sdd/$ARGUMENTS` + domain keywords, `project: "{project}"` — recover implementation context and find patterns from prior reviews
...
**CRITICAL**: For the blocked path (SPEC-GAP-HIGH), run this step BEFORE returning `Status: blocked`. The session lifecycle mirror (Step 0) runs AFTER the result envelope.
