...
`/implement-task` computes `TDD_MODE` per task (see `.claude/agents/sdd-implement-task.md` → "TDD detection (hard rule)"). It is **ON** automatically whenever the repo has a test framework configured OR any test files exist — no declaration needed. When ON, test-first (a real failing test before implementation) is mandatory for every task with testable behavior; the only escape is a non-testable task (infra/config/migration/docs) with a `## Test-skip rationale` entry in `decisions.md`.

Use the knob below only to **force** the stance explicitly:

- `tdd: strict` — force `TDD_MODE` ON even in a greenfield repo with no tests yet (the implementer must introduce the framework and write the failing test first).
