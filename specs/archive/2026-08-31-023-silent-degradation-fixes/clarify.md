...
Script de shell versionado; cada arreglo es aditivo (una línea impresa, un warning, campos nuevos en `.pr-opened`, reglas de parseo). Ninguno migra datos ni cambia formato de forma incompatible, y el único lector de `.pr-opened` (`bin/sdd:1240`) solo verifica existencia. `git revert` alcanza; no hace falta flag.

### Q: El ítem de cobertura ES uno de los seis, no solo cómo verificamos los otros cinco. Propuse cuatro ejes del espacio de entrada (fin de línea; estructura del documento; resolución; estado del índice) y que el spec los liste como criterio de aceptación, para que el reviewer verifique "¿está cubierto el eje?" en vez de "¿pasa el test?" — la distinción que falló en 022, cuando un test verde tapó una AC incumplida. Junto con eso presenté el quality gate completo (8 GWT, los 5 unchanged, rollback y success metric).
Recommended answer: los cuatro ejes tal cual, listados en el spec como AC8; los cinco intocables como guardarraíl de regresión; rollback por revert sin flag; success metric = para los próximos features archivados con sidecar, `gh pr view --json baseRefName` coincide con `.parent-branch` sin intervención manual, y cero resoluciones de base sin procedencia impresa.
ok

### Q (lane): ¿seguimos con el ítem 1 solo (fast-lane) o los seis?
Recommended answer: —
fixiemos todo. creo que es momento de parar despues de eso, y ver que hacemos con este proyecto, pero mas con el core
⇒ seis ítems, más de 2 GWT y más de un dominio ⇒ full-spec, no fast-lane.
