...
- [ ] Given `.claude/rules/conventions.md` § Domain rules contains distinctive domain names, When a consuming agent fills an artifact's domain section, Then those exact names appear in the output; and Given the section is emptied and the artifact regenerated, Then the names are absent and the section is scan-derived.
