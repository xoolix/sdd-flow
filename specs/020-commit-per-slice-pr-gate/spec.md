...
| Surface | Contract |
|---|---|
| `sdd commit-slice <id> --task Tnnn --files <p>…` | Stages listed paths **plus** `specs/<id>/`; commits as `<type>(<id>): Tnnn <title>`. Exits ≠0 without `--files`. Never pushes, never `git add -A`. |
...
| `tasks.md` slice metadata | New `type:` field (`feat`/`fix`/`refactor`/`chore`) beside `blocked_by`/`verifies`/`touches`, written by `sdd-task-planner`. |
...
| Unrelated dirty files in tree | Never staged — only `--files` plus `specs/<id>/`. |
